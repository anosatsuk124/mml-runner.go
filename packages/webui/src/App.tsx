// via https://github.com/jdachtera/solid-pianoroll/blob/main/dev/Demo.tsx
...
const App: Component = () => {
  ...
  const parts = mapArray(
    ...
    (_, trackIndex) => {
      ...
      return new Tone.Part(
        (time, noteIndex) => {
          const note = untrack(
            () => pianoRollState.tracks?.[trackIndex()]?.notes[noteIndex],
          );
          if (!note) return;

          Tone.Draw.schedule(
            () => pianoRollState.onNoteDown(trackIndex(), note.midi),
            time,
          );
  ...
  createEffect(() => {
    ...
    pianoRollState.onDurationChange(
      longestTrackLength || pianoRollState.ppq * 4,
    );
  });
  ...
  const updatePlayHeadPosition = () => {
    if (pianoRollState.playHeadPosition === previousPlayheadPosition) {
      pianoRollState.onPlayHeadPositionChange(
        TransportTime(transport.seconds).toTicks(),
      );
    }
        ...
        <div>
          ...
          <select
            ...
            onChange={(event) => {
              ...
              pianoRollState.onGridDivisionChange(
                +option.value as GridDivision,
              );
            }}
          >
        ...
        <div>
          <label>
            <input
              ...
              onChange={() =>
                pianoRollState.onSnapToGridChange(!pianoRollState.snapToGrid)
              }
            />
            Snap to grid
          </label>
